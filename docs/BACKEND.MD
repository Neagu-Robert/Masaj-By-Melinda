...
| `log_admin_action_on_profile()` | `profiles` | `AFTER UPDATE OR DELETE` | Inserts a row into `admin_audit_logs` for admin-initiated ban/unban/delete actions. Actions: `user.ban`, `user.unban`, `user.delete`. |
